...
def findMicroInversionsList(lstReads):
	for el in lstReads:
		findMicroInversions(el)

def manageReads(lstReads):
	global numProcs
	numReads = len(lstReads)
	if numReads > 0:
		readsPerProc = numReads // numProcs
		leftover = numReads % numProcs
		pos = 0
		lstForProc = {}
		...
		for i in range(0, numProcs):
			lstForProc[i] = []
			if leftover > 0:
				for j in range(pos, readsPerProc+pos+1):
					lstForProc[i].append(lstReads[j])
				pos += readsPerProc
				leftover -= 1
			else:
				for j in range(pos, readsPerProc + pos):
					lstForProc[i].append(lstReads[j])
				pos += readsPerProc
		finalLstforProcs = []
		for el in lstForProc:
			finalLstforProcs.append(lstForProc[el])
		p.map(findMicroInversionsList, finalLstforProcs)
		#p.close()
		#p.join()
	else:
