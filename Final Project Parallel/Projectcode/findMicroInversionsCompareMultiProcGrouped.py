...
def checkDicMultiProc(invRead, charDensity):
    ...
    if (charDensity[0], charDensity[1], charDensity[2], charDensity[3], charDensity[4]) in sequencesDic:
        ...
        lstEl = []
        elPerProc = numEl // numProcs
        leftover = numEl % numProcs
        pos = 0
        for i in range(0, numProcs):
            indNums = []
            if leftover > 0:
                for j in range(pos, elPerProc+pos+1):
                    indNums.append(j)
                pos += elPerProc
                leftover -= 1
            else:
                for j in range(pos, elPerProc+pos):
                    indNums.append(j)
                pos += elPerProc
            lstEl.append((indNums, sequencesDic[(charDensity[0], charDensity[1], charDensity[2], charDensity[3], charDensity[4])], invRead.strip('\n')))
...
def checkDictionary(tup):
    ...
    invRead = tup[2]
    for pos in el:
        if sequence[indices[pos]:indices[pos]+READ_LEN] == invRead:
            return (True, indices[pos])
    return (False, 0)
